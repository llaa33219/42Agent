...
def verify_dependencies() -> tuple[bool, list[str]]:
    ...
    packages = ["websockets", "dashscope", "PyQt6", "pyaudio", "PIL", "chromadb", "sentence_transformers", "vncdotool"]
    broken = []
    for pkg in packages:
...
def repair_dependencies(broken: list[str]) -> bool:
    ...
    pkg_map = {"PIL": "Pillow", "sentence_transformers": "sentence-transformers"}
    for pkg in broken:
